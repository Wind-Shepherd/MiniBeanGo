...
public:
	...
	int x_left, y_bottom; // 改为int类型
	int vx, vy;           // 改为int类型
	int gravity;          // 改为int类型
	int width, height;    // 改为int类型
	...
	void runRight(Scene& scene, bool switchAnimationOnly) // 新的重载函数，接受两个参数
	{
		if (switchAnimationOnly) // 如果switchAnimationOnly为true，仅切换动画
		{
			if (playerStatus != standright) // 如果之前角色状态不是向右奔跑
			{
				playerStatus = standright; // 切换为向右奔跑状态
		...
		else // 如果switchAnimationOnly为false，执行向右奔跑
		{
			runRight(scene); // 调用现有的runRight函数
		}
	}
	...
	void runLeft(Scene& scene, bool switchAnimationOnly) // 新的重载函数，接受两个参数
	{
		if (switchAnimationOnly) // 如果switchAnimationOnly为true，仅切换动画
		{
			if (playerStatus != standleft) // 如果之前角色状态不是向左奔跑
		...
		else // 如果switchAnimationOnly为false，执行向左奔跑
		{
			runLeft(scene); // 调用现有的runLeft函数
		}
	}
	...
	int isNotOnAllLands(vector<Land>& lands, int speed) // 判断玩家是否不在所有的地面上
	{
