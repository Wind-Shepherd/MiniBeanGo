...
public:
	...
	void drawClippedFrame()
	{
		...
		if (spriteImage != nullptr)
		{
			...
			// 渲染到屏幕
			putimagePng(x_left, y_bottom - frameH, &tempImg);
		}
	}
	...
	int isOnLand(Land& land, float ySpeed)
	{
		...
		if (ySpeed <= 0) // y轴方向速度小于0，表示正在向上运动，不需要考虑速度的影响
			ySpeed = 0;
		if (land.left_x - x_left <= width * 0.6 && x_right - land.right_x <= width * 0.6 && abs(y_bottom - land.top_y) <= +ySpeed)
			return 1;
		else
			return 0;
	}
