...
public:
	YouWinUI() : exitRequested(false), gameOverImg(NULL) {
		lastTime = clock();

		// 修复备用图片创建逻辑
		if (_waccess(L"YouWinImg.jpg", 0) == 0) {
			gameOverImg = new IMAGE;
			loadimage(gameOverImg, L"YouWinImg.jpg", 200, 200);
		}
		else {
			// 创建空图像
			gameOverImg = new IMAGE(200, 200);
	...
	void draw() {
		...
		settextcolor(LIGHT_COLOR);
		settextstyle(20, 0, L"Arial");
		wstring subtitle = L"The adventure is to be continued...";
		int subtitleWidth = textwidth(subtitle.c_str());
		int subtitleX = (WINDOW_WIDTH - subtitleWidth) / 2;
		outtextxy(subtitleX, 160, subtitle.c_str());
...
void show()  // 显示
{
	...
	healthHeart.draw(player.healthStatus); // 显示血量条
	//player.drawHealthStatus();
	FlushBatchDraw(); // 批量绘制
...
int main() // 主函数
{
	...
	if (game.run()) {
		...
		while (1)       // 一直循环
		{
			...
			if (!player.isNotOnAllGroundThorns(scene.groundthorns)|| !player.isNotOnAllEnemys(scene.enemys))
			{
				player.reduceHealth(true);
			}

			if (player.isOnTheKey(scene.keys) != -1)
			{
				int i = player.isOnTheKey(scene.keys);
				scene.keys[i].setVisibility(0);
				player.key=1;
			}

			if (player.isOnTheStar(scene.stars) != -1)
			{
				int i = player.isOnTheStar(scene.stars);
				scene.stars[i].setVisibility(0);
			}

			if (player.isOnTheGate(scene.gates) != -1)
			{
				if (player.key) {
					player.win = 1;
				}
			...
			if (_kbhit() && GetAsyncKeyState('F') & 0x8000) // 检测 'F' 键是否按下（F键为互动键）
			{
				if (player.isOnTheRod(scene.rods) != -1)
				{
					int i = player.isOnTheRod(scene.rods);
					scene.rods[i].changeStatus();
					scene.update(i);
				}
			}
		}
		...
		if (!c) {
			initgraph(WINDOW_WIDTH, WINDOW_HEIGHT);
			SetWindowText(GetHWnd(), L"MiniBeanGo - You Win");
